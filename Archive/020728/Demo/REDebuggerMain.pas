...
type
  TfmREDebuggerMain = class(TForm)
   ...
   protected
    fHelpFolder : string;
    fShowOnce : boolean;
    procedure AssignTestCase (ACase : TRETestCase);
   public
...
uses
 ...
 PCode, RETestCasesDlg;

const
 ProductHomePage = 'anso.virtualave.net';
...
procedure TfmREDebuggerMain.AssignTestCase (ACase : TRETestCase);
 begin
  if not Assigned (ACase) then begin
    Application.MessageBox (
  ...
  with ACase do begin
    ...
    edInputString.Text := InputString;
    memSubstitutionTemplate.Text := SubstitutionTemplate;
    edReplaceString.Text := String4Replace;
    // chkUseSubstitution.Checked := ;
    r.ModifierStr := ModifierStr;
    UpdateModifiers;
   end;
 end;
...
procedure TfmREDebuggerMain.FormShow(Sender: TObject);
 ...
 begin
  ...
  with fmRETestCasesDlg do begin
    Idx := REs.IndexByName ('EMail');
    if Idx < 0
     then Idx := 0;
    if (Idx >= 0) and (Idx < REs.Count) then begin
      grdREs.Row := Idx + 1;
      AssignTestCase (RE);
     end;
   end;
