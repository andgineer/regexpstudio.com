...
---------------------------------------------------------------
French translation of ErrorMsg (used with ErrorMsg function) by
---------------------------------------------------------------
     Martin Ledoux
     Quebec, Canada
     mledoux@citenet.net
...
function TRegExpr.ErrorMsg (AErrorID : integer) : RegExprString;
 begin
  case AErrorID of
    ...
    reeCompNullArgument: Result := 'TRegExpr(comp): Argument Nul';
    reeCompRegexpTooBig: Result := 'TRegExpr(comp): ExpRég Trop Grosse';
    reeCompParseRegTooManyBrackets: Result := 'TRegExpr(comp): Trop de ParseReg ()';
    reeCompParseRegUnmatchedBrackets: Result := 'TRegExpr(comp): ParseReg Incomparable ()';
    reeCompParseRegUnmatchedBrackets2: Result := 'TRegExpr(comp): ParseReg Incomparable ()';
    reeCompParseRegJunkOnEnd: Result := 'TRegExpr(comp): ParseReg bute sur la fin';
    reePlusStarOperandCouldBeEmpty: Result := 'TRegExpr(comp): *+ Operérateur Peut-Être Vide';
    reeNestedSQP: Result := 'TRegExpr(comp): Emboîté *?+';
    reeBadHexDigit: Result := 'TRegExpr(comp): Mauvais Chiffre Hex';
    reeInvalidRange: Result := 'TRegExpr(comp): Mauvaise Portée de []';
    reeParseAtomTrailingBackSlash: Result := 'TRegExpr(comp): Atome d''Analyse \ Traînant';
    reeNoHexCodeAfterBSlashX: Result := 'TRegExpr(comp): Pas de Chiffre Hex Après \x';
    reeHexCodeAfterBSlashXTooBig: Result := 'TRegExpr(comp): Chiffre Hex Après \x Trop Gros';
    reeUnmatchedSqBrackets: Result := 'TRegExpr(comp): [] Incomparable';
    reeInternalUrp: Result := 'TRegExpr(comp): Urp Interne';
    reeQPSBFollowsNothing: Result := 'TRegExpr(comp): ?+*{ Suit Rien';
    reeTrailingBackSlash: Result := 'TRegExpr(comp): \ Traînant';
    reeRarseAtomInternalDisaster: Result := 'TRegExpr(comp): RarseAtom Désastre Interne';
    reeBRACESArgTooBig: Result := 'TRegExpr(comp): BRACES Argument Trop Gros';
    reeBracesMinParamGreaterMax: Result := 'TRegExpr(comp): BRACE Param Min Plus Grand Que Max';
    reeUnclosedComment: Result := 'TRegExpr(comp): Non-Fermé (?#Comment)';
    reeComplexBracesNotImplemented: Result := 'TRegExpr(comp): Si vous voulez prendre part aux test beta de BRACES ''{min,max}'' et des ops non-vorace ''*?'', ''+?'', ''??'' pour les cas complexe - enlever ''.'' de {.$DEFINE ComplexBraces}';
    reeUrecognizedModifier: Result := 'TRegExpr(comp): Modificateur Non-Reconnu';
    reeBadLinePairedSeparator: Result := 'TRegExpr(comp): LinePairedSeparator doit contenir 2 différent caractère ou rien du tout';

    reeRegRepeatCalledInappropriately: Result := 'TRegExpr(exec): RegRepeat Appel Inaproprié';
    reeMatchPrimMemoryCorruption: Result := 'TRegExpr(exec): MatchPrim Corruption de Mémoire';
    reeMatchPrimCorruptedPointers: Result := 'TRegExpr(exec): MatchPrim Pointeurs Corrompus';
    reeNoExpression: Result := 'TRegExpr(exec): Propriété d''Expression pas validé';
    reeCorruptedProgram: Result := 'TRegExpr(exec): Programme Corrompu';
    reeNoInpitStringSpecified: Result := 'TRegExpr(exec): Pas de Chaîne d''entrée Spécifiée';
    reeOffsetMustBeGreaterThen0: Result := 'TRegExpr(exec): Le Décalage Doit-Être Plus Grand Que 0';
    reeExecNextWithoutExec: Result := 'TRegExpr(exec): ExecNext Sans Exec[Pos]';
    reeGetInputStringWithoutInputString: Result := 'TRegExpr(exec): GetInputString Sans InputString';
    reeDumpCorruptedOpcode: Result := 'TRegExpr(dump): Opcode Corrompu';
    reeLoopStackExceeded: Result := 'TRegExpr(exec): Pile de Loupe Excédée';
    reeLoopWithoutEntry: Result := 'TRegExpr(exec): Loop Sans LoopEntry !';
