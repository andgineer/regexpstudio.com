...
{.$IFDEF VER145}{$IFDEF LINUX}{$IF CompilerVersion = 14.5} {$DEFINE D7} {$DEFINE D6} {$DEFINE D5} {$DEFINE D4} {$DEFINE D3} {$DEFINE D2} {$IFEND}{$ENDIF} // Kylix 3
...
const
 TRegExprVersionMajor : integer = 1;
 TRegExprVersionMinor : integer = 95;
...
class function TRegExpr.InvertCaseFunction (const Ch : REChar) : REChar;
 begin
  ...
  {$ENDIF}
   begin
{$IFDEF LINUX}
    Result := {$IFDEF FPC}AnsiUpperCase (Ch) [1]{$ELSE} REChar (toupper(integer(Ch))){$ENDIF};
{$ELSE}
    Result := {$IFDEF FPC}AnsiUpperCase (Ch) [1]{$ELSE} REChar (CharUpper (PChar (Ch))){$ENDIF};
{$ENDIF}
    if Result = Ch
{$IFDEF LINUX}
     then Result := {$IFDEF FPC}AnsiLowerCase (Ch) [1]{$ELSE} REChar (tolower (integer (Ch))){$ENDIF};
{$ELSE}
